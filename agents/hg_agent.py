...
hg_agent.py
-----------
Mercurial Version Control Agent — Phase 3 of the AI automation pipeline.
...
If not set, it's a no-op — existing pipeline flow is unchanged.
...
def _slugify(text: str, max_words: int = 6) -> str:
    ...
    'Create SLA breach notification for incident requests'
      → 'CREATE_SLA_BREACH_NOTIFICATION_INCIDENT'
    """
    ...
    def _commit_to_branch(self, state: AgentState, cfg: dict) -> dict:
        ...
        if not repo_root or not Path(repo_root).exists():
            return {
                ...
                "message": "Repo root not found — hg workflow skipped.",
            }
        ...
        # (files may be outside the hg repo — skip those)
        ...
        if not files_to_commit:
            return {
                ...
                "pushed": False,
                "files_committed": [],
                "message": "No Java file changes detected in the hg repo — nothing to commit.",
                "error": "",
            }
        ...
        if should_push:
            ...
            if push_result.returncode == 0:
                ...
                # returncode=1 from hg push = "no changes to push" — not really an error
